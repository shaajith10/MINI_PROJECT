...
const ChatRoom = () => {
  ...
  const handleSendMessage = async () => {
    ...
    const messages = {
      ...
      timestamp: new Date(),
      type: "text",
      readBy: [user?.email || "unknown"],
      reactions: {},
      replyTo: replyTo || null
    };

    try {
      await addDoc(collection(db, "messages"), messages);
      setNewMessage("");
      setReplyTo(null);
    } catch (err) {
        ...
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          >
            ...
            {messages?.map((message) => {
              const isOwn = message.senderEmail === user?.email; // check who sent the message
                    ...
                    {message.type === 'file' && (
                      <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
                        ...
                        <IconButton
                          ...
                          sx={{ color: isOwn ? 'white' : 'black', ml: 1 }}
                        >
